...
class Inkscape extends Stream {
  constructor(inkscapeArgs, inkscapeBinLocation = '') {
    super();

    this.inkscapeBinLocation = inkscapeBinLocation
    ...
    this.commandLine = `${this.inkscapeBinLocation}inkscape${
  ...
  write(chunk) {
    ...
    this.writeStream.once('close', () => {
      ...
      this.inkscapeProcess = childProcess.spawn(`${this.inkscapeBinLocation}inkscape`, this.inkscapeArgs);
      const stdoutChunks = [];
      const stderrChunks = [];
